...
io.on('connection', function(socket) {
    ...
    socket.on('start-moving-players', function(direction) {
    ...
    socket.on('stop-moving-player', function() {
        if(players[socket.id]){
            if(games[players[socket.id].gameId].players.length != 2) {
                return;
            }
    ...
    socket.on('disconnect', function() {
        ...
        if (players[socket.id]) {
            const gameId = players[socketId].gameId;
            const game = games[gameId];
            const playersToRemove = game.players.map(function(player) {
            return player.socketId;
            })
            clearInterval(game.gameInterval);
            delete games[gameId];
            playersToRemoveIds.forEach( function(playersToRemove) {
    ...
    socket.on('attack', function () {
        if(players[socket.id]){
            ...
            game.bullets.push(new Buller(players[socket.id]));
        }
    })
})
