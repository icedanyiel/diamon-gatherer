...
// TEMA 3
document.getElementById('increaseCounter').addEventListener('click', () => { //do this when button is clicked
    const counter = document.getElementById("currentCounter");
    let nCount = +counter.innerHTML;
    nCount++;
    socket.emit("increase-counter", nCount);
});
socket.on('get-current-counter', function(counter) { //listener function
    document.getElementById('currentCounter').innerHTML = counter;
})
//END