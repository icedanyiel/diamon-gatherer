...
class Player {
    ...
    move() {
        ...
        if (newX != this.x && newX > 0 && (newX + PLAYER_DIM.width) < MAP.width){
            ...
            this.stept = Math.floor(this.x / PLAYER_DIM.width % 2);
        }
        if (newX != this.x && newX > 0 && (newX + PLAYER_DIM.height) < MAP.height){
            this.y = this.y + this.dy;
            this.stept = Math.floor(this.y / PLAYER_DIM.height % 2);
        }
    }
